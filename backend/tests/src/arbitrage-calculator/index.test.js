...
describe('Test get input from orderbooks', () => {
    test('should return object with id and corresponding orderbook', () => {
        const orderbooks = {
            'BTC-EUR': {
                state() {
                    return {
        ...
        const products = calculator.getInputFromOrderbooks(orderbooks)
        expect(products[0]).toEqual({ id: 'BTC-EUR', orderbook:{
                                        'asks': [{price: BigNumber(8005.33), size:BigNumber(0.1)}], 
                                        'bids': [{price: BigNumber(7999), size:BigNumber(1.3)}]
                                    }})
        expect(products[1]).toEqual({ id: 'ETH-BTC', orderbook:{
                                        'asks': [{price: BigNumber(0.23), size:BigNumber(4.3)}], 
                                        'bids': [{price: BigNumber(0.19), size:BigNumber(17)}]
                                    }})
        expect(products[2]).toEqual({ id: 'ETH-EUR', orderbook:{
                                        'asks': [{price: BigNumber(231.5), size:BigNumber(1.2)}], 
                                        'bids': [{price: BigNumber(230.65), size:BigNumber(0.4)}]
                                    }})
    })
